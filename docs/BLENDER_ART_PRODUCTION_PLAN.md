...
| 遺物 | 86 | 真鍮機械と台座で比較的統一。第一弾4個はBlender原本あり | 機構キット11種を共有し、台座なしの透過アイコンにする |
...
最終アート方向は「工業的なクロノ・ファンタジー」とします。黒鉄、青みのある鋼、古い真鍮、白いセラミック、革、ガラス、液体、控えめなシアン・琥珀・赤の発光を共通語彙にします。カードは効果を発揮する瞬間を描く場面イラストとし、背景・構図・照明をカードごとに変えます。遺物は物体そのものを大きく見せる台座なしの透過アイコン、人物は同じ世界の装甲・機械兵として統一します。
...
| `art_src/blender/library/qq_render_rigs.blend` | カードごとのカメラ・照明プリセットと、遺物・ポートレート・UI用リグ |
...
- `iron_plating`: 重装胸甲と留め具を単体の透過アイコンとして配置。
...
| C12 | 人物用のhalo、crown、wings、spear、R01の装甲部品 |
...
| 出力 | 原本レンダー | 最終出力 | 規則 |
| --- | --- | --- | --- |
| カード | 2048x2048、カード固有の場面・背景 | 512x512 RGBA | 枠内いっぱいの構図、文字・カード名・枠なし、主効果を中央へ置く |
| 遺物 | 2048x2048、透過フィルム | 512x512 RGBA | 台座・床・背景なし、周囲に透明な余白、名前なし |
...
カラーマネジメントと材質の基準は共有し、カードの露出・焦点距離・背景・照明は場面に合わせて設定します。各設定は再生成可能なプリセットとして保存し、高解像度レンダーから高品質縮小します。第一弾revision 02はEEVEEで制作しています。
...
### 第一弾の構図改訂（2026-09-07 / QQ-0.20.1）

共通の展示台・背景による単調さを解消するため、カード用台座を廃止しました。
今後のカード制作にも個別の場面・背景・カメラを適用し、遺物は台座なしの透過画像とします。

| カード | 改訂した場面 |
| --- | --- |
| `quick_slash` | 廃墟を背景に、剣を振るう手と斬撃を対角線の接写で配置 |
| `guard` | 雨の門で、構えた盾が飛来する攻撃を受け止める |
| `delay_step` | 夕暮れの時間空間で、大時計の前に敵の剣を拘束 |
| `repair_burst` | 木々の中で、ナノマシンが腕の装甲を修復 |
| `auto_turret` | 瓦礫の戦場に展開し、薬莢を排出しながら射撃 |
| `event_horizon` | 星雲を背景に、破片を取り込む重力場と降着流 |
