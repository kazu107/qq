...
7. 準備画面のロードアウト右側には、全対戦者の名前、スコア、準備状態を1人1行で表示する。準備中は黄色、準備完了は緑で表示する。
8. 全対戦参加者が準備を完了すると最初の組み合わせの戦闘へ進む。
9. 戦闘画面では対戦する2人が`戦闘開始`を押し、3秒カウントダウン後に開始する。
10. ラウンド内の全組み合わせが終了すると、次の準備フェーズまたは規定勝利数到達による最終結果へ進む。
