...
12. 対戦の勝敗にかかわらず、同じラウンド基本ゴールドを両者へ付与する。敗者は次の準備フェーズに向けてHPを全回復する。勝者だけが受け取る選択報酬と、遺物固有の追加報酬は別途適用する。
13. ロビーの`対戦情報`から、順位、最新ラウンド結果、各プレイヤーのHP・ゴールド・所持カード・装備カード・遺物を前面ウィンドウで確認できる。大会終了後、または観戦終了後にロビーへ戻った場合は、このウィンドウを自動表示する。
