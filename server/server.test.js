...
test("host and guest receive stable peer ids and relay signaling", async () => {
  ...
  send(host, { type: "host", room: "ABC123", protocolVersion: 8, contentHash: BUILD_HASH });
...
test("room directory tracks compatible rooms, player counts, and host rules", async () => {
  ...
  const host = await connect(app.port);
  const guest = await connect(app.port);
  send(directory, { type: "list_rooms", protocolVersion: 8, contentHash: BUILD_HASH });
...
test("room validation rejects missing, mismatched, and full rooms", async () => {
  ...
  send(missing, { type: "join", room: "NOPE00", protocolVersion: 8, contentHash: BUILD_HASH });
  assert.equal((await receive(missing)).code, "room_not_found");

  send(host, { type: "host", room: "ROOM42", protocolVersion: 8, contentHash: BUILD_HASH });
  await receive(host);
  const mismatch = await connect(app.port);
  send(mismatch, { type: "join", room: "ROOM42", protocolVersion: 9, contentHash: BUILD_HASH });
...
test("even-capacity rooms accept multiple players and separate spectators", async () => {
  ...
  for (let index = 0; index < 3; index += 1) {
    ...
    assert.deepEqual(assigned.peers, [1]);
    assert.equal((await hostPeerJoined).id, index + 2);
  }
  ...
  assert.deepEqual(spectatorAssigned.peers, [1]);
  assert.equal((await hostSpectatorJoined).id, 5);
