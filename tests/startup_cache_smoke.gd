...
func _run() -> void:
	...
	or int(summary.get("battle_3d_models", 0)) < 8 \
	or BattleActor3D.get_cached_authored_model_count() < 8 \
	or int(summary.get("battle_animations", 0)) != 18 \
	or BattleAnimationCatalog.get_load_error() != "":
