...
if ($godotExe) {
    ...
    Invoke-GodotCheck "[28/31] Running LAN multiplayer protocol smoke" $godotExe @(
        "--no-header",
        "--headless",
        "--path", $root,
        "--scene", "res://tests/LanMultiplayerSmoke.tscn"
    )
    Invoke-GodotCheck "[29/31] Running online-disabled local clone smoke" $godotExe @(
        "--no-header",
        "--headless",
        "--path", $root,
        "--scene", "res://tests/OnlineDisabledSmoke.tscn"
    )
    Write-Host "[30/31] Running LAN two-peer ENet smoke"
    ...
    Write-Host "[31/31] Running LAN sustained network soak"
