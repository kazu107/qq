...
const DEFAULT_OUTPUT_DIR: String = "res://assets/icons/relics"
...
func _initialize() -> void:
	...
	var relic_ids: Array[String] = _load_relic_ids()
	if manifest.is_empty() or relic_ids.is_empty():
		quit(1)
		return
	...
	if allow_partial:
		for manifest_id_value: Variant in manifest.keys():
			...
			if not relic_ids.has(manifest_id):
				_fail("Generated art manifest contains an unknown relic: %s" % manifest_id)
				return
		for relic_id: String in relic_ids:
			if manifest.has(relic_id):
				import_ids.append(relic_id)
	elif manifest.size() != relic_ids.size():
		_fail("Generated art manifest has %d entries, but relics.json has %d relics" % [
			manifest.size(),
			relic_ids.size(),
		])
		return
		...
		for relic_id: String in relic_ids:
			if not manifest.has(relic_id):
				_fail("Generated art manifest is missing relic: %s" % relic_id)
				return
			import_ids.append(relic_id)

	var mkdir_error: Error = DirAccess.make_dir_recursive_absolute(output_dir)
	if mkdir_error != OK:
		_fail("Could not create relic art output directory: %s" % output_dir)
		return

	var processed_images: Array[Image] = []
	for relic_id: String in import_ids:
		var source_path: String = String(manifest.get(relic_id, ""))
		if source_path.is_empty() or not FileAccess.file_exists(source_path):
			_fail("Missing generated source image for relic: %s" % relic_id)
			return

		var image: Image = Image.new()
		var load_error: Error = image.load(source_path)
		if load_error != OK or image.is_empty():
			_fail("Could not load generated source for %s: %s" % [relic_id, source_path])
			return
		...
		var target_path: String = output_dir.path_join("%s.png" % relic_id)
		var save_error: Error = image.save_png(target_path)
		if save_error != OK:
			_fail("Could not save generated relic art: %s" % target_path)
			return
	...
	if not contact_sheet_path.is_empty():
		...
		if contact_error != OK:
			_fail("Could not save relic art contact sheet: %s" % contact_sheet_path)
			return

	print("Imported %d generated relic images at %dx%d." % [
...
func _load_relic_ids() -> Array[String]:
	var file: FileAccess = FileAccess.open(RELIC_DATA_PATH, FileAccess.READ)
	if file == null:
		push_error("Could not open relic data: %s" % RELIC_DATA_PATH)
		return []
	var parsed: Variant = JSON.parse_string(file.get_as_text())
	if not parsed is Array:
		push_error("Relic data must be a JSON array")
		return []

	var relic_ids: Array[String] = []
	for entry_value: Variant in parsed as Array:
		if not entry_value is Dictionary:
			push_error("Relic data contains a non-object entry")
			return []
		var entry: Dictionary = entry_value as Dictionary
		var relic_id: String = String(entry.get("id", ""))
		if relic_id.is_empty() or relic_ids.has(relic_id):
			push_error("Relic data contains an empty or duplicate id: %s" % relic_id)
			return []
